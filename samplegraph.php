...
chart5.draw(dataTable5, options5);
    ...
    var departmentCheckboxes = document.getElementById('departmentCheckboxes');
    departments.forEach(function(department, index) {
        ...
        departmentCheckboxes.appendChild(checkbox);
        departmentCheckboxes.appendChild(label);
        departmentCheckboxes.appendChild(document.createElement('br'));
    });

    // Function to update chart based on selected departments
    function updateChart(data5, departments) {
        ...
        newData.forEach(function(item) {
            var row = [monthNames[item.month_number - 1]];
            selectedDepartments.forEach(function(department) {
                var value = (item.department === department) ? parseInt(item.bookings_count) : 0;
                row.push(value);
            });
        ...
        selectedDepartments.forEach(function(department) {
            dataTable.addColumn('number', department);
        });
        dataTable.addRows(newDataArray);

        var chart = new google.visualization.LineChart(document.getElementById('chart5'));
        chart.draw(dataTable, options5);
    }
    //--end--//
