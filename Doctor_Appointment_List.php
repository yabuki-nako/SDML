...
if ($result1->num_rows > 0) {
  while ($row = $result1->fetch_assoc()) {
    ...
    echo "<td><button type='button' class='btn btn-success btn-accept' onclick=\"openModal('" . $row['appointment_ID'] . "', '" . $row['pID'] . "','" . $row['pname'] . "', '".$row['App_status']."','".$row['appDate']."')\" data-toggle='modal' data-target='#exampleModal'>Add Health Records</button>";
      ...
      <form action='<?php htmlspecialchars($_SERVER["PHP_SELF"])?>' method='post' onsubmit="return validateForm()">
        <input type='hidden' name='action' value='input1'>
          ...
          <div class="form-group">
            ...
            <input type="number" class="form-control" id="bmi_Modal" name="bmi_Modal" placeholder ="" readonly>
          ...
          <div class="form-group">
            <label for="pBloodType_Modal" class="col-form-label">Blood type</label>
              <select name="pBloodType" id="pBloodType" name="pBloodType" class="form-control form-control-lg">
                  <option value = "0" disabled selected>Select Bloodtype</option>
    ...
    <div class="input-group">
        ...
        <input type="text" class="form-control" id="diastolic_BP" name="diastolic_BP" placeholder="Diastolic">
          ...
          <div class="form-group">
            ...
            <input type="number" class="form-control" id="respiratoryRate_Modal" name="respiratoryRate_Modal" min="30.00" max ="99" step="0.01" placeholder ="">
          </div>
        <div class="form-group">
            <label for="temp_Modal" class="col-form-label">Temperatory (Celcius)</label>
            <input type="number" class="form-control" id="temp_Modal" name="temp_Modal" min="30.00" max ="99" step="0.01" placeholder ="">
          </div>
...
<script>
  function validateForm() {
    // Get the value of the textarea and height input
    var note = document.getElementById('note_Modal').value.trim();
    var pheight = document.getElementById('pHeight_Modal').value.trim();
    var pweight = document.getElementById('pWeight_Modal').value.trim();
    var pBmi = document.getElementById('bmi_Modal').value.trim();
    var pBp= document.getElementById('bp_Modal').value.trim();
    var pResp = document.getElementById('respiratoryRate_Modal').value.trim();
    var pTemp = document.getElementById('temp_Modal').value.trim();
    ...
    if (note === '') {
      document.getElementById('note_Modal').placeholder = 'Please provide blood pressure.';
      errorMessage += 'Please provide a note.\n';
    }

    // Check if the height input is empty or not a number
    if (pheight === '' || isNaN(parseInt(pheight))) {
      document.getElementById('pHeight_Modal').placeholder = 'Please provide blood pressure.';
      errorMessage += 'Please insert a valid height.\n';
    }
    if (pweight === '' || isNaN(parseInt(pheight))) {
      document.getElementById('pWeight_Modal').placeholder = 'Please provide blood pressure.';
      errorMessage += 'Please insert a valid weight.\n';
    }
    if (pBmi === '' || isNaN(parseInt(pBmi))) {
      document.getElementById('bmi_Modal').placeholder = 'Please provide blood pressure.';
      errorMessage += 'Please insert a valid BMI.\n';
    }
    if (pBp === '' || isNaN(parseInt(pBp))) {
      document.getElementById('bp_Modal').placeholder = 'Please provide blood pressure.';
      errorMessage += 'Please insert a valid Blood Pressure.\n';
    }
    if (pResp === '' || isNaN(parseInt(pResp))) {
      document.getElementById('respiratoryRate_Modal').placeholder = 'Please provide blood pressure.';
      errorMessage += 'Please insert a valid Resp.\n';
    }
    if (pTemp === '' || isNaN(parseInt(pTemp))) {
      document.getElementById('temp_Modal').placeholder = 'Please provide blood pressure.';
      errorMessage += 'Please insert a valid Temp.\n';
    }
    if (pDrug === '' || isNaN(parseInt(pDrug))) {
      document.getElementById('drug_Modal').placeholder = 'Please provide blood pressure.';
      errorMessage += 'Please insert a valid Drug.\n';
    }
    if (bloodType === '0') {
      document.getElementById('pBloodType').placeholder = 'Please provide blood pressure.';
      errorMessage += 'Please select a blood type.\n';
    }
    // Check if there are any error messages
    if (errorMessage !== '') {
      // Display the concatenated error messages in a single alert box
      alert(errorMessage);
